...
def int_quake_cat2dict(**kwargs):
    ...
    json_string_1    = kwargs.get('json1', None)
    json_string_2    = kwargs.get('json2', None)
    ...
    # Event Ids
    try:
        origin_id      =  str(json_string_2['features'][0]['properties']['originid'])
    except:
        origin_id      =  str(json_string_2['features'][0]['properties']['originId'])

    event_id      =  str(json_string_2['features'][0]['properties']['eventId'])
    author        =  str(json_string_2['features'][0]['properties']['author'])
    version       =  str(json_string_2['features'][0]['properties']['version'])

    # Epicenter informations from SISZ_ED file
    lon           =  float(json_string_1['alerts'][0]['longitude'])
    lat           =  float(json_string_1['alerts'][0]['latitude'])
    depth         =  float(json_string_1['alerts'][0]['depth'])/1000.0 ### in meters
    OT            =  str(json_string_1['alerts'][0]['time'])
    mag           =  float(json_string_1['alerts'][0]['magnitude'])

    # Epicenter information from default file
    area          =  str(json_string_2['features'][0]['properties']['place'])
    ev_type       =  str(json_string_2['features'][0]['properties']['type'])
    mag_type      =  str(json_string_2['features'][0]['properties']['magType'])
    ...
    OT_tmp = OT.replace(", ","/").split('/')
    OT_fin = OT_tmp[2] + "-" + OT_tmp[1] + "-" + OT_tmp[0] + "T" + OT_tmp[3]
    OT = OT_fin
    ...
    try:
        mag_percentiles = json_string_2['features'][0]['properties']['mag_percentiles']
    except:
    ...
    try:
        cov_matrix      = json_string_2['features'][0]['properties']['cov_matrix']
    except:
        #'cov_matrix' : {'XX' : '1.70816', 'XY' : '0.270061', 'XZ' : '0.0988157', 'YY' : '2.74951', 'YZ' : '0.48637', 'ZZ' : '8.04068'},
        # From Zante
        cov_matrix = dict()
        cov_matrix['XX'] = 10.000
        cov_matrix['XY'] = 0.0
        cov_matrix['XZ'] = 0.0
        cov_matrix['YY'] = 10.000
        cov_matrix['YZ'] = 0.0
        cov_matrix['ZZ'] = 10.000
...
def load_event_parameters(**kwargs):
    ...
    event_ingv  = kwargs.get('event_ingv',None)
    ...
    if(json_rabbit != None):
        ...
        s1 = open(event, 'r').read()
        jsn_object_1 = eval(s1)
        s2 = open(event_ingv, 'r').read()  
        jsn_object_2 = eval(s2)

    else:

        return False

    event_dictionary = int_quake_cat2dict(json1=jsn_object_1,json2=jsn_object_2, routing_key=routing_key, args=args)
