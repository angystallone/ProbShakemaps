...
def get_pois(POIs_File, Lon_Event, Lat_Event, POIs_Subset, pois_selection_method, n_pois, max_distance):
    ...
    if POIs_Subset == False:
        ...
        return POIs_lat, POIs_lon, POIs_NAMES     
    ...
    if POIs_Subset == True:
        ...
        if pois_selection_method == 'random':
            ...
            for _ in range(n_pois):
                ...
                random_index = random.randint(0, len(LONs) -1)
                random_lon = LONs[random_index]
                random_lat = LATs[random_index]

                dist = dist_lonlat(random_lon, random_lat, Lon_Event, Lat_Event, 'degree')

                # if the distance is greater than MAX_DISTANCE, choose another random point
                if dist > max_distance:
                    continue

                if random_lon not in POIs_lon and random_lat not in POIs_lat:
                    idx_POIs.append(random_index)
                    POIs_lat.append(random_lat)
                    POIs_lon.append(random_lon)
                    POIs_NAMES.append(f"Site_LAT:{Point(float(random_lon),float(random_lat)).latitude}_LON:{Point(float(random_lon),float(random_lat)).longitude}")      
        ...
        if pois_selection_method == 'azimuth_uniform':
            ...
            idx_POIs, POIs_lat, POIs_lon, POIs_NAMES = [], [], [], []
            unique_lat, unique_lon = [], []
            azimuths = []

            azimuth_range = np.linspace(-180, 180, n_pois, endpoint=False)

            for _ in range(n_pois):
                for idx, (lon, lat) in enumerate(zip(LONs, LATs)):

                    # calculate the distance and azimuth from the event
                    distance = dist_lonlat(lon, lat, Lon_Event, Lat_Event, 'degree')
                    azimuth, _, _ = geod.inv(lat, lon, Lat_Event, Lon_Event)
                    ...
                    if not idx_POIs:
                        if max_distance - DIST_BUFFER <= distance <= max_distance + DIST_BUFFER:
                            ...
                            unique_lon.append(lon)
                    else:
                        if lon not in POIs_lon and lat not in POIs_lat:
                            # get the closest segment index to the azimuth angle
                            segment_idx = int(np.argmin(np.abs(azimuth - azimuth_range)))
                            # check if the segment is available
                            if max_distance - DIST_BUFFER <= distance <= max_distance + DIST_BUFFER and all(abs(azimuth - azimuths[i]) > 180/n_pois for i in range(len(idx_POIs)) for azimuth in azimuth_range[segment_idx:segment_idx+1]):
                                azimuths.append(azimuth)
                                idx_POIs.append(idx)
                                POIs_lat.append(lat)
                                POIs_lon.append(lon)
...
class StationRecords:
    ...
    def plot(self):
        ...
        sc = m.scatter(x, y, c=np.log10(data_imt), vmin=np.log10(self.imt_min), vmax=np.log10(self.imt_max), s=1, cmap = cm)
        cbar = plt.colorbar(sc)
        cbar.set_label(f"log10({self.imt})")
        plt.show()

        path = os.path.join(os.getcwd(), "OUTPUT")
        figname = f"/Data_stationfile_{self.imt}.png"
...
class QueryHDF5:
    def __init__(self, output, Lon_Event, Lat_Event, scenario, pois_file, pois_subset, n_pois, max_distance, pois_selection_method):
        ...
        if self.pois_subset == False:  
            ...
            self.POIs_lat, self.POIs_lon, self.POIs_NAMES = get_pois(self.pois_file, self.Lon_Event, self.Lat_Event, self.pois_subset, 
    ...
    def plot_distributions(self):
        ...
        for iPoi in ipois:
            ...
            if datum > 0 and datum_found > 0:
                plt.show()
                fig.savefig(path + '/' + 'Distr_POI-' + "{:03d}".format(iPoi + 1) + '.png', dpi = 200)
            else:
